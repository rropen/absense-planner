var calendarClickToggle = false;
var token = document.currentScript.getAttribute("token");

//Send the data to the backend
function sendData(ID, date, half_day, half_day_time, type) {
    var data = JSON.stringify({"id": ID, "date": date, "half_day": half_day, "half_day_time": half_day_time, "type": type})
    if (type == "add") {
        ...
        if (e.target.id.includes("/")) {
            ...
            //Remove an absnece
                ...
                document.getElementById("removeAbsence").onclick = function() {
                    sendData(ID, date, false, "N", "remove")
