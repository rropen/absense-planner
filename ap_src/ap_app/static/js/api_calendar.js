...
function filterTeams(input) {
  var calendars = Array.from(
    document.getElementById("calendar-group").children
  );
  for (cal in calendars) {
    var calendarID = calendars[cal].id.toString();
    calendarID = calendarID.replace("title-", "");
    if (
      !calendarID.toUpperCase().includes(input.value.toString().toUpperCase())
    ) {
...
function setDate(e, id) {
  ...
  if (id == 0) {
    window.location.replace(
      window.location.origin + `/calendar/${month}/${year}`
    );
  } else {
    window.location.replace(
      window.location.origin + `/teams/api-calendar/${id}/${month}/${year}`
    );
  }
}
