...
def teams_dashboard(request) -> render:
    try:
        ...
        r = requests.get(
            env("TEAM_DATA_URL")
            + "api/user/teams/?username={}".format(request.user.username),
            headers={"TEAMS-TOKEN": encryption},
        )
    except:
        return render(request, "teams/dashboard.html", {"external_teams": False})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            response = requests.post(
                env("TEAM_DATA_URL") + "api/teams/?format=json", data=request.POST
            )
...
def join_team(request) -> render:
    ...
    if userprofile.external_teams:
        try:
            r = requests.get(
                env("TEAM_DATA_URL")
                + "api/teams/?username={}".format(request.user.username)
            )
        except:
...
def edit_team(request: HttpRequest, id):
    ...
    return render(
        ...
        context={"team": api_data[0], "roles": roles, "url": env("TEAM_DATA_URL")},
    )
...
def edit_api_data(userprofile, id):
    ...
    if userprofile.external_teams:
        ...
        raise NotImplementedError(
            "The API setting is not enabled in your profile. (No error page)"
        )
